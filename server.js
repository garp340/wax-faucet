...
const FAUCET_ACCOUNT = process.env.WAX_FAUCET_ACCOUNT?.trim().toLowerCase();
const WAX_ENDPOINT = process.env.WAX_ENDPOINT?.trim() || 'https://wax.greymass.com';
...
// Evitar que el faucet se envíe a sí mismo
...
try {
    ...
    console.log(
        `Claim enviado: ${CLAIM_AMOUNT} -> ${cleanAccount} | ${result.transaction_id}`
    );
...
console.log(`Cuenta faucet: ${FAUCET_ACCOUNT || 'NO CONFIGURADA'}`);
console.log(`WAX endpoint: ${WAX_ENDPOINT}`);
console.log(`Clave privada: ${PRIVATE_KEY ? 'CONFIGURADA' : 'NO CONFIGURADA'}`);
});
